...
func NewRouter(authHandler *AuthHandler, groupsHandler *GroupsHandler, collectionsHandler *CollectionsHandler, paymentsHandler *PaymentsHandler, formsHandler *FormsHandler, authService middleware.AccessTokenParser) *gin.Engine {
	...
	groups.GET(":group_id/subscription", middleware.RequireAuth(authService), groupsHandler.GetSubscriptionSummary)
	groups.POST(":group_id/subscription/checkout", middleware.RequireAuth(authService), groupsHandler.CreateSubscriptionCheckoutSession)
	groups.POST(":group_id/subscription/confirm", middleware.RequireAuth(authService), groupsHandler.ConfirmSubscriptionCheckoutSession)
	...
	api.GET("/payments/:payment_id/qrcode", middleware.RequireAuth(authService), paymentsHandler.GetPaymentQRCode)
